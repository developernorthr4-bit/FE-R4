...
import { fiscalLabel, getPmSummary, type PmSummary } from '../services/maintenance.api'
import {
...
<template>
  <AppLayout>
    ...
    <!--
      ...
      เนื้อหาเต็มย้ายไปหน้า /maintenance แล้ว เพราะ PM ผูกกับปีงบประมาณ
      ไม่ใช่สัปดาห์ที่เลือกด้านบน — เหลือไว้ตรงนี้สามตัวเลขให้ผู้บริหารที่เปิด
      แค่แดชบอร์ดยังรู้ว่ามีเรื่องนี้อยู่และกดเข้าไปดูต่อได้
    -->
    ...
    <div class="card border border-base-300 bg-base-100">
      <div class="card-body gap-3">
        <div class="flex flex-wrap items-baseline justify-between gap-2">
          <h2 class="card-title text-base">งานบำรุงรักษาเชิงป้องกัน (PM)</h2>
          <span v-if="pm" class="text-sm opacity-70">
            ปีงบ {{ fiscalLabel(pm.fiscalYear) }} · ไม่ขึ้นกับสัปดาห์ที่เลือกด้านบน
          </span>
        </div>

        <p v-if="pmError" class="text-sm opacity-70">{{ pmError }}</p>
        <span v-else-if="pmLoading" class="loading loading-spinner loading-sm opacity-60" />

        <template v-else-if="pm">
          <div class="flex flex-wrap items-center gap-x-10 gap-y-3">
            <div>
              <p class="text-xs opacity-70">ตู้ที่ตรวจแล้วในปีงบนี้</p>
              <p class="text-2xl font-semibold tabular-nums">
                {{ pm.progress.done.toLocaleString() }}<span
                  class="text-sm opacity-50"
                >/{{ pm.progress.cabinetsTotal.toLocaleString() }}</span>
                <span class="ml-1.5 text-sm opacity-70">{{ pmPct }}%</span>
              </p>
            </div>
            <div>
              <p class="text-xs opacity-70">SOH ต่ำกว่า 70%</p>
              <p class="text-2xl font-semibold tabular-nums">
                {{ pm.batteries.lowSoh.toLocaleString() }}
              </p>
            </div>
            <div>
              <p class="text-xs opacity-70">ตำหนิทางกายภาพ</p>
              <p
                class="text-2xl font-semibold tabular-nums"
                :class="pm.batteries.defectBanks > 0 ? 'text-error' : ''"
              >
                {{ pm.batteries.defectBanks.toLocaleString() }}
              </p>
            </div>
          </div>

          <div class="card-actions">
            <RouterLink to="/maintenance" class="btn btn-sm btn-outline">
              ดูทะเบียนตู้/แบตเตอรี่ และผลตรวจทั้งหมด
            </RouterLink>
          </div>
        </template>
      </div>
