...
/**
 * แผงจัดการของในสถานี — ตู้ / อุปกรณ์ / แบตเตอรี่
 *
 * เป็นแผงเลื่อนทับหน้าเดิม ไม่ใช่หน้าใหม่ เพราะงานจริงคือไล่แก้ทีละสถานีตาม
 * ตัวกรองที่ตั้งไว้ในตาราง ถ้าเด้งไปหน้าใหม่ทุกครั้งจะเสียตัวกรองกับเลขหน้าทุกรอบ
 *
...
const emit = defineEmits<{
  close: []
  /** บอกตารางแม่ว่าจำนวนตู้/อุปกรณ์/แบตเปลี่ยนแล้ว ให้โหลดใหม่ */
  changed: []
}>()
...
type PendingDelete = {
  ...
  id: string
  label: string
  /** สิ่งที่จะหายไปด้วย — บรรทัดละอย่าง ให้อ่านออกก่อนกด ไม่ใช่ถามซ้ำเปล่า ๆ */
  losses: string[]
...
<template>
  <div class="fixed inset-0 z-50 flex justify-end">
      ...
      <header class="flex items-start justify-between gap-3 border-b border-base-300 p-4">
        <div>
          <h2 class="text-lg font-semibold">
            ตู้ อุปกรณ์ และแบตเตอรี่
            <span v-if="data" class="opacity-70">· {{ data.site.siteCode }}</span>
          </h2>
          <p v-if="data" class="text-sm opacity-70">
            {{ data.site.siteName ?? 'ไม่มีชื่อสถานี' }} · {{ data.site.provinceName }}
          </p>
        </div>
              ...
              <table class="table table-xs">
                <tbody>
                  ...
                  <tr v-for="e in looseEquipments" :key="e.id">
                    ...
                    <td v-if="canEdit" class="text-right">
                      <button
                        type="button" class="btn btn-xs btn-ghost" @click="openEditEquipment(e)"
                      >
                        ย้ายเข้าตู้
                      </button>
                      <button
      ...
      <div class="modal-box max-w-2xl">
        ...
        <form class="mt-4 grid gap-3 sm:grid-cols-2" @submit.prevent="submit">
          ...
          <label v-if="form.kind !== 'battery'" class="form-control">
            ...
            <input
              v-model="form.serialNo" type="text" class="input input-sm input-bordered w-full"
            />
          </label>
          ...
          <div class="modal-action sm:col-span-2">
            <button
              type="button" class="btn btn-ghost" :disabled="saving" @click="form = null"
            >
              ยกเลิก
            </button>
    ...
    <!-- ═══ ยืนยันลบ ═══ -->
    <div
      v-if="pendingDelete" class="modal modal-open" @click.self="pendingDelete = null"
    >
      <div class="modal-box">
        ...
        <template v-else>
          <div v-if="pendingDelete.losses.length" role="alert" class="alert alert-error mt-4 text-sm">
            <span>
              สิ่งที่จะหายไปด้วย: {{ pendingDelete.losses.join(' · ') }}
            </span>
          </div>
